...
def stitch_tiles_vertical(tile1, tile2, overlap):
    ...
    centroids1_overlap = np.array([np.round(r.centroid).astype(int) for r in regions1_overlap])
    ...
    centroids2_overlap = np.array([np.round(r.centroid).astype(int) for r in regions2_overlap])
    ...
    centroids1_leftover = np.array([np.round(r.centroid).astype(int) for r in regions1_leftover])
    ...
    if len(upper_edge_unique) > 0:
        for unique_edge in upper_edge_unique:
            position_to_check = np.rint(np.median(np.where(final_overlap[0,:] == unique_edge))).astype(int)
    ...
    if len(lower_edge_unique) > 0:
        for unique_edge in lower_edge_unique:
            position_to_check = np.rint(np.median(np.where(final_overlap[-1,:] == unique_edge))).astype(int)
