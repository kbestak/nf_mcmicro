...
class PyramidWriter:
    ...
    def __init__(
            self, _in_path, _out_path, _markers, _channels, _nuclear_channels, _membrane_channels, _max_projection, _x, _y, _x2, _y2, _w, _h, scale=2, tile_size=1024, peak_size=1024,
            verbose=False
    ):
        if tile_size % 16 != 0:
...
def validate_segmentation_channels(df):
    segmentation_channels = df["segmentation_channel"].dropna().astype(int).tolist()

    # Ensure values are only 0, 1, or 2, and appear at most once
    if any(ch > 2 for ch in segmentation_channels):
        raise ValueError("Segmentation channel contains values greater than 2")
    if len(segmentation_channels) == 0:
        raise ValueError("No segmentation channels specified")
    if (1 in segmentation_channels or 2 in segmentation_channels) and 0 not in segmentation_channels:
        raise ValueError("If 1 or 2 appear, 0 must also be present")
    if len(set(segmentation_channels)) != len(segmentation_channels):
        raise ValueError("Segmentation channels must contain unique values (0, 1, 2 at most)")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--markers', type=str, required=True, help="Marker Sheet Path")
    ...
    parser.add_argument('--version', action='version', version='placeholderv2.0.0')
